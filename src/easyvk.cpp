...
namespace easyvk {
	...
	Buffer::Buffer(easyvk::Device &_device, uint32_t _size) :
		...
		size(_size)
		{
			...
			vkGetPhysicalDeviceMemoryProperties(device.device, &memoryProperties);

			// Determine the memory type that suits your requirements
			...
			memoryAllocateInfo.memoryTypeIndex = suitableMemoryTypeIndex; // Get this from memory properties

			VkDeviceMemory memory;
			vkAllocateMemory(device, &memoryAllocateInfo, nullptr, &memory);

			// Step 2: Create Buffer and Bind Memory
			VkBufferCreateInfo bufferCreateInfo = { VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO };
			bufferCreateInfo.size = size; // Specify buffer size
			...
			vkMapMemory(_device.device, memory, 0, size, 0, &newData);
			data = (uint32_t*)newData;


			// Allocate memory with suitable properties (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT and others)
			// VkMemoryAllocateInfo memoryAllocateInfo = { VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO };
			// memoryAllocateInfo.allocationSize = size; // Size of memory region
			// memoryAllocateInfo.memoryTypeIndex = suitableMemoryTypeIndex; // Get this from memory properties
			// vkAllocateMemory(device, &memoryAllocateInfo, nullptr, &memory);
			...
			// // Map the memory for device access
			// void* deviceMappedData = nullptr;
			// vkMapMemory(_device.device, memory, 0, size, 0, &deviceMappedData);

			// // Map the memory for host access
			// void* hostMappedData = nullptr;
			// vkMapMemory(_device.device, memory, 0, size, VK_MEMORY_MAP_FLAG_HOST, &hostMappedData);

*/
            // Allocate and map memory to new buffer
	      auto memId = _device.selectMemory(buffer, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_CACHED_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
